...
public class Board {
    ...
    private final List<List<Cell>> array = new ArrayList<>();
    ...
    private void setValue(int column, int row) {
    ...
    public void findZeroes(int column, int row) {
        if (array.get(column).get(row).getValue() == 0 && !array.get(column).get(row).isBomb()) {
    ...
    public void showBombs() {
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                if (array.get(i).get(j).isBomb()) array.get(i).get(j).cellClicked();
            }
        }
    }
}